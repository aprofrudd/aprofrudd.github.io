...
/* ---------------------------------------------------------------------------
   Section 01 — the two attention charts, then what they mean together.
   ------------------------------------------------------------------------ */
function focus(mount) {
  mount.appendChild(h('div', { class: 'v-prose' },
    h('p', { html:
      'VO&#8322;max is having a moment. It turns up on watches, in podcasts, in longevity books ' +
      'and in gym marketing. The obvious assumption is that scientists found something new. ' +
      'They did not. Here are the two halves of the story side by side.'
    })
  ));
  ...
  // The synthesis. Both charts stand alone, so this is where they are put
  // together — and the numbers are computed, not typed.
  const peakRow = PUBMED_SERIES.reduce((a, b) =>
    ((b.vo2 / b.total) > (a.vo2 / a.total) ? b : a));
  const lastRow = PUBMED_SERIES[PUBMED_SERIES.length - 1];

  const synthesis = h('div', { class: 'v-callout' });
  synthesis.innerHTML =
    '<span class="v-callout-head">Put the two together</span>' +
    `Research attention to fitness climbed for four decades, peaked in <strong>${peakRow.year}</strong> ` +
    `and has not risen since &mdash; it is lower today than it was then. Public attention went the ` +
...
/* ---------------------------------------------------------------------------
   Section 08 — takeaways. Rewritten to cover only what the page still shows.
   ------------------------------------------------------------------------ */
function takeaways(mount) {
  ...
  mount.appendChild(h('div', { class: 'v-grid' },
    [
      ...
      ['Under 5 METs is the number to remember',
       'Below about 5 METs means struggling to sustain a brisk walk. Above 8 METs means being able to jog. Most of the difference in this study sits between those two.'],
  ...
  mount.appendChild(h('p', { class: 'v-caption', style: 'margin-top:2rem' },
    'Every figure here was rebuilt from the numbers in the paper; none of the original artwork is ' +
    'reproduced. Where a value could only be read off a published chart rather than a printed table, ' +
    'it is labelled as approximate. Publication counts come from PubMed via the NCBI E-utilities API, ' +
    `fetched ${PUBMED_META.fetched}.`
  ));
}
