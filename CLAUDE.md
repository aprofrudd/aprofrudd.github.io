...
vo2max/
  ...
  01-.../
    ...
    cite.js           refLink/sourceCard: every paper linked one way (DOI, else PubMed)
...
- **Sections 10–13 draw on eight other papers, all in `SOURCES` in `data.js`, read from PubMed abstracts only** (`abstractOnly: true`; the source card says so). A number the paper holds but the abstract does not is a typed `null` (`BEDREST.fallPct`, `JOYNER.economy`) or an empty array (`RECORDS.groups`), and the UI degrades in a stated way — an open-ended bar with a "?", a box with a "?", a panel that does not render. When the PDF arrives, fill the field; nothing else changes. Flags: `derived` (arithmetic on printed numbers, shown in the comment), `approx` (a "~" in the source), `estimated` (an estimate in the source), `hypothetical` (a modelled person). Averages of products are not products of averages — the Fick presets print the paper's measured value beside the computed one on purpose.
